...
#Trains the machine learning penetrance prediction models for splice change variants. The final output is a table of penetrance scores and a set of trained models.


num_splits=5


splice_table=pd.read_pickle('/Path/to/ML/Model/Data/SpliceChangeExpressionData.pth')
...
penetrance_score_table = pd.DataFrame([],columns=['RF_Score','LogReg_Score','RF_Expressed_Flag','LogReg_Expressed_Flag','RF_Aymptomatic_Flag','LogReg_Aymptomatic_Flag'],index=splice_table.index)
...
logreg_output_table.to_csv('/Path/to/ML/Model/Results/SpliceChangeLogRegPerformanceTable.txt',sep='\t')
rf_output_table.to_csv('/Path/to/ML/Model/Results/SpliceChangeRandomForestPerformanceTable.txt',sep='\t')

# global models 
rf_mod = RandomForestClassifier(min_samples_leaf=5,min_samples_split=10,criterion='log_loss',n_estimators=500)
final_rf_model=rf_mod.fit(design_matrix.values,is_expressed.values)
dump(final_rf_model, '/Path/to/ML/Model/Models/SpliceChangeRandomForestModel.pth') 

final_lin_mod=LogisticRegression(C=1.0,penalty='l2',fit_intercept=False).fit(design_matrix.values,is_expressed.values)
dump(final_lin_mod, '/Path/to/ML/Model/Models/SpliceChangeLogisticRegressionModel.pth') 

biologic_artifact_score_table.to_csv('/Path/to/ML/Model/Results/SpliceChangePenetranceScores.txt',sep='\t')
biologic_artifact_score_table.to_pickle('/Path/to/ML/Model/Results/SpliceChangePenetranceScores.pth')
