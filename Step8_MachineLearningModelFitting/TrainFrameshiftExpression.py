...
#Trains the machine learning penetrance prediction models for frameshift variants. The final output is a table of penetrance scores and a set of trained models.

num_splits=5
frameshift_table=pd.read_pickle('/Path/to/ML/Model/Data/FrameshiftExpressionData.pth')
frameshift_table=frameshift_table.loc[frameshift_table.IN_HQ_SET==True]
...
is_expressed = frameshift_table.BINARIZED_EXPRESSION
...
penetrance_score_table = pd.DataFrame([],columns=['RF_Score','LogReg_Score','RF_Expressed_Flag','LogReg_Expressed_Flag','RF_Aymptomatic_Flag','LogReg_Aymptomatic_Flag'],index=frameshift_table.index)



np.random.seed(1212019)
...
logreg_output_table.to_csv('/Path/to/ML/Model/Results/FrameshiftLogRegPerformanceTable.txt',sep='\t')
rf_output_table.to_csv('/Path/to/ML/Model/Results/FrameshiftRandomForestPerformanceTable.txt',sep='\t')

# global models 
rf_mod = RandomForestClassifier(min_samples_leaf=5,min_samples_split=10,criterion='log_loss',n_estimators=500)
final_rf_model=rf_mod.fit(design_matrix.values,is_expressed.values)
dump(final_rf_model, '/Path/to/ML/Model/Models/FrameshiftRandomForestModel.pth') 

final_lin_mod=LogisticRegression(C=1.0,penalty='l2',fit_intercept=False).fit(design_matrix.values,is_expressed.values)
dump(final_lin_mod, '/Path/to/ML/Model/Models/FrameshiftLogisticRegressionModel.pth') 

penetrance_score_table.to_csv('/Path/to/ML/Model/Results/FrameshiftPenetranceScores.txt',sep='\t')
penetrance_score_table.to_pickle('/Path/to/ML/Model/Results/FrameshiftPenetranceScores.pth')
