...
# #This script computes the pLoF prevalence risk ratios for each haploinsufficient disease. It requires the following input information:
# 1) Table of basic disease information (disease_table)
# 2) Table containing the biobank covariates (age at enrollment, sex, ancestry PCs; must be indexed by sample ID)
# 3) Path to the pLoF-carrier tables created using Step3_Build_Carrier_and_NonCarrier_Datasets/BuildCarrier_NonCarrier_Datasets.py
# 4) Table of rare disease diagnoses, with one row per biobank subject and a simple binary indicator ('HasDisDx') that indicates whether the subject carries a disease-specific diagnosis code for this disorder. See CollapsedDxTable.txt in Auxillary_Data/CollapsedDxTable.txt for list of disease-specific diagnostic codes 
...
output_table_txt.to_csv('/Path/to/Basic/Penetrance/Results/DxCodeAssociationAnalysis.txt',sep='\t')
