...
frameshift_table=pd.read_pickle('../Results/FrameshiftExpressionData.pth')
...
biologic_artifact_score_table = pd.DataFrame([],columns=['RF_Score','LogReg_Score','RF_Expressed_Flag','LogReg_Expressed_Flag','RF_Aymptomatic_Flag','LogReg_Aymptomatic_Flag'],index=frameshift_table.index)
...
logreg_output_table.to_csv('../Results/FrameshiftLogRegPerformanceTable.txt',sep='\t')
rf_output_table.to_csv('../Results/FrameshiftRandomForestPerformanceTable.txt',sep='\t')

# global models 
rf_mod = RandomForestClassifier(min_samples_leaf=5,min_samples_split=10,criterion='log_loss',n_estimators=500)
final_rf_model=rf_mod.fit(design_matrix.values,is_expressed.values)
dump(final_rf_model, '../Models/FrameshiftRandomForestModel.pth') 

final_lin_mod=LogisticRegression(C=1.0,penalty='l2',fit_intercept=False).fit(design_matrix.values,is_expressed.values)
dump(final_lin_mod, '../Models/FrameshiftLogisticRegressionModel.pth') 

biologic_artifact_score_table.to_csv('../Results/FrameshiftArtifactScores.txt',sep='\t')
biologic_artifact_score_table.to_pickle('../Results/FrameshiftArtifactScores.pth')
