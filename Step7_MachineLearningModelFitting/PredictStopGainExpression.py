...
biologic_artifact_score_table = pd.DataFrame([],columns=['RF_Score','LogReg_Score','RF_Expressed_Flag','LogReg_Expressed_Flag','RF_Aymptomatic_Flag','LogReg_Aymptomatic_Flag'],index=stop_gain_table.index)
...
logreg_output_table.to_csv('../Results/StopGainLogRegPerformanceTable.txt',sep='\t')
rf_output_table.to_csv('../Results/StopGainRandomForestPerformanceTable.txt',sep='\t')

# global models 
rf_mod = RandomForestClassifier(min_samples_leaf=5,min_samples_split=10,criterion='log_loss',n_estimators=500)
final_rf_model=rf_mod.fit(design_matrix.values,is_expressed.values)
dump(final_rf_model, '../Models/StopGainRandomForestModel.pth') 
...
dump(final_lin_mod, '../Models/StopGainLogisticRegressionModel.pth') 

biologic_artifact_score_table.to_csv('../Results/StopGainArtifactScores.txt',sep='\t')
biologic_artifact_score_table.to_pickle('../Results/StopGainArtifactScores.pth')
